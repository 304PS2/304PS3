---
title: "Predicting 2020 American Federal Election by logistic regression model with post-stratification technique"
...
Here we are interested in predicting the popular vote outcome of the 2020 American federal election (Tausanovitch & Lynn, 2020).  To do this we are employing a logistic regression model .  Historically speaking, the voting outcome differs significantly by region and race. To take into account this difference in sample and target population, we are employing a post-stratification technique to decrease the variance and bias of the predictors selected in the model. 
...
$$ log(\frac{y_{i}}{1-y_{i}}) = \beta_{o}+\beta_1x_{age} + \beta_2x_{sex}+\beta_3x_{race} + \beta_4x_{region}+ \epsilon$$
...
In order to determine the significance of  four independent variable (age, sex, race and region), we apply an ANOVA table for this logistic regression model. From Table 2, it illustrates that the residual deviance of the null model with just an intercept is 7197.8, every addition of  predictors decreases the residual deviance. For instance, when age is a predictor, the residual deviance decreases from 7197.8 to 7130.8. Similarly, as sex, race and region of voter are predictors, the residual deviance decreases into 7040.2, 6645.2, and 6604.6.
...
In conclusion, we think the Democratic party will win the primary vote since the estimated proportion of voters in favor of voting for him is 0.5282 which is bigger than the proportion of the republican party.
